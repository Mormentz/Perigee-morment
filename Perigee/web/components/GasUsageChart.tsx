...
export const GasUsageChart: React.FC<GasUsageChartProps> = React.memo(
  ({
    ...
    // Stabilize the chart configuration object so the chart only re-renders
    // when the underlying metrics actually change (FE-016).
    ...
    const maxValue = useMemo(
      () => Math.max(...metrics.map((m) => Math.max(m.simulated, m.average)), 1),
      [metrics],
    );
